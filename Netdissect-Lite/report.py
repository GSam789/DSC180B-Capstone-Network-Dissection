...
import expdir as expdir
import bargraph as bargraph
import settings
import numpy as np
...
def generate_html_summary(ds, layer, maxfeature=None, features=None, thresholds=None,
    ...
    for label_order, record in enumerate(rendered_order):
        ...
        if force or not ed.has('html/%s' % imfn):
            ...
            for x, index in enumerate(top[unit]):
                ...
                print('image shape: ' + str(image.shape[:2]))
                print('feature: ' + str(features[index][unit]))
                mask = np.array(Image.fromarray(obj=features[index][unit], mode='F').resize(image.shape[:2]))
                print('mask: ' + str(mask))
                print('threshold: ' + str(thresholds[unit]))
    ...
    with open(ed.filename('html/image/%s.html' % expdir.fn_safe(layer)), 'w') as f:
        f.write('\n'.join(html))
