...
class SegmentationPrefetcher:
    ...
    def next_job(self):
        if self.index < 0:
            ...
            print("self.index: length of index: " + str(self.index))
            return None
        ...
        if self.index >= len(self.indexes):
            print("length of indexes: " + str(len(self.indexes)))
            print("length of index: " + str(self.index))
            if self.once:
                print("once excuted")
                self.index = -1
            else:
                print("once not excuted")
                self.index = 0
                if self.randomize:
                    # Reshuffle every time through
                    self.random.shuffle(self.indexes)
        print("self.index right before function return: " + str(self.index))
        return result
    ...
    def fetch_batch(self):
        ...
        try:
            ...
            if len(self.result_queue) == 0:
                print("length of result_queue: " + str(len(self.result_queue)))
                print("fetch batch")
                return None
