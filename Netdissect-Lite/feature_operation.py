...
import settings
import time
import upsample as upsample
import vecquantile as vecquantile
import multiprocessing.pool as pool
from data_loader import load_csv
from data_loader import SegmentationData, SegmentationPrefetcher
                ...
                for concept_map in batch:
                    ...
                    for unit_id in range(units):
                        ...
                        if feature_map.max() > threshold[unit_id]:
                            mask = np.array(Image.fromarray(obj=feature_map, mode='F').resize((concept_map['sw'], concept_map['sh'])))
