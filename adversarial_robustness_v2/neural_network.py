...
class NeuralNetwork():
  ...
  def cross_entropy_with(self, y):
    return tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=tf.stop_gradient(y)))
  ...
  def logps(self):
    ...
    return self.logits - tf.reduce_logsumexp(input_tensor=self.logits, axis=1, keepdims=True)
  ...
  def certainty_sensitivity(self):
    ...
    return tf.gradients(ys=crossent_w_1, xs=self.X)[0]
  ...
  def grad_sum_logps(self):
    ...
    return tf.gradients(ys=self.logps, xs=self.X)[0]

  @cachedproperty
  def l1_certainty_sensitivity(self):
    """Cache the L1 loss of that product"""
    return tf.reduce_mean(input_tensor=tf.abs(self.certainty_sensitivity))
  ...
  def l1_weights(self):
    ...
    return tf.add_n([tf.reduce_sum(input_tensor=tf.abs(v)) for v in tf.compat.v1.trainable_variables() if v in self.vars])

  @cachedproperty
  def l2_weights(self):
    """L2 loss for the weights of the network"""
    return tf.add_n([tf.nn.l2_loss(v) for v in tf.compat.v1.trainable_variables() if v in self.vars])
  ...
  def cross_entropy_grads(self):
    ...
    return tf.gradients(ys=self.cross_entropy, xs=self.X)[0]

  @cachedproperty
  def l1_double_backprop(self):
    """L1 loss of the sensitivity of the cross entropy"""
    return tf.reduce_sum(input_tensor=tf.abs(self.cross_entropy_grads))
  ...
  def binary_logit_grads(self):
    return tf.gradients(ys=self.binary_logits, xs=self.X)[0]

  @cachedproperty
  def accuracy(self):
    return tf.reduce_mean(input_tensor=tf.cast(tf.equal(
      self.preds, tf.argmax(input=self.y, axis=1)), dtype=tf.float32))
  ...
  def loss_function(self,
    ...
    if softmax_temperature == 1:
      ...
      log_likelihood = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=self.logits/softmax_temperature, labels=tf.stop_gradient(self.y)))
    log_prior = 0
    for reg in [
  ...
  def adversarial_training_loss(self, fgsm_eps, clip_min, clip_max,
    ...
    """
    preds_max = tf.reduce_max(input_tensor=self.logits, axis=1, keepdims=True)
    y_symb = tf.stop_gradient(tf.cast(tf.equal(self.logits, preds_max), dtype=tf.float32))
    y_symb = y_symb / tf.reduce_sum(input_tensor=y_symb, axis=1, keepdims=True)
    adv_loss = self.cross_entropy_with(y_symb)
    loss_grad, = tf.gradients(ys=adv_loss, xs=self.X)
    ...
    logits2 = model2[-1]
    cross_entropy2 = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits=logits2, labels=tf.stop_gradient(self.y)))
    loss = cross_entropy2
    if l2_double_backprop > 0:
      xent2_grads = tf.gradients(ys=cross_entropy2, xs=X_adv)[0]
