...
def LecunLCN(X, image_shape, threshold=1e-4, radius=7, use_divisor=True):
    ...
    convout = tf.nn.conv2d(input=X, filters=filters, strides=[1,1,1,1], padding='SAME')
    ...
    if use_divisor:
        ...
        # Compute variances
        sum_sqr_XX = tf.nn.conv2d(input=tf.square(centered_X), filters=filters, strides=[1,1,1,1], padding='SAME')

        # Take square root to get local standard deviation
        denom = tf.sqrt(sum_sqr_XX)

        per_img_mean = tf.reduce_mean(input_tensor=denom)
...
def gaussian_filter(kernel_shape):
    ...
    return tf.convert_to_tensor(value=x / np.sum(x), dtype=tf.float32)
