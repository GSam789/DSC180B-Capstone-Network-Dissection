...
def test_perclass_pra(model, dataset,
    ...
    if ablated_units is not None:
        def ablate_the_units(x, *args):
            x[:,ablated_units] = x[:,ablated_units] / 4
            return x
...
def test_weight_change(model, dataset,
        layername=None, change_units=None, sample_size=None, cachefile=None):
    '''Classifier precision/recall/accuracy measurement.
    change the weights of units in the specified layer, and then
    measures per-class precision, recall, accuracy and
    balanced (binary classification) accuracy for each class,
    compared to the ground truth in the given dataset.'''
    try:
    ...
    if change_units is not None:
        def units_weight_change(x, *args):
            x[:,change_units] = x[:,change_units] / 2
            return x
        model.edit_layer(layername, rule=units_weight_change)
    with torch.no_grad():
        num_classes = len(dataset.classes)
        true_counts = torch.zeros(num_classes, dtype=torch.int64).cuda()
        pred_counts = torch.zeros(num_classes, dtype=torch.int64).cuda()
...
def sharedfile(fn):
    ...
    filename = os.path.join('drive/MyDrive/results', fn)
